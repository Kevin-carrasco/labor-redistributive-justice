...
Matter survey (19,169 respondents in 27 countries), this article
...
cumulative-link models with random country intercepts. Three
individual-level expectations hold: concern about losing one's job or
income and the perception that others receive undeserved benefits
depress perceived justice, while perceiving technology as enabling
raises it. Two do not: platform workers judge their exchange as more
just rather than less, and perceived technological displacement risk
increases rather than erodes perceived justice. Welfare regimes do not
differ on average, but the moderation analyses reveal a consistent
...
on these seven items yields a two-factor structure: three displacement
items load on a first factor, three benefit items on a second, and the
replacement by AI doesn't load well, capturing an internal element of
risk perception distinct from the two external ones. Two scales are
therefore built ---Technological Displacement Risk (the average of the
three displacement items) and Perceived Technological Benefit (the
average of the three benefit items)---. In all cases, higher values
indicate a greater perception of the corresponding risk or benefit. The
full exploratory factor analysis is reported in the appendix at
Table~\ref{tbl-efa}.
...
are estimated sequentially: type of employment (Model 1), adding job
insecurity (Model 2), the perceptions of technological risk and benefit
(Model 3), the perceived deservingness of others (Model 4), the type of
welfare regime (Model 5) and the control variables (Model 6).

Model 7 introduces interaction terms between the labour market
predictors, and Models 8 to 12 add interactions with the type of welfare
regime to test the moderation hypotheses. All the analyses use the
individual survey weights provided by the OECD. The models are estimated
with the clmm function of the ordinal package in R.
...
platform worker (r = 0.32, p \textless{} 0.001), technological
displacement (r = 0.24, p \textless{} 0.001), skill obsolescence risk (r
= 0.26, p \textless{} 0.001) and perceived technological benefit (r =
0.21, p \textless{} 0.001), all of them positive and moderate.
...
and with the technological perception variables (r = 0.18; r = 0.3). The
correlations among these three technological variables are themselves
positive and significant (r = 0.42; r = 0.49), and the strongest of
them, between technological displacement and perceived benefit (r =
0.72), runs contrary to the initial intuition that risk and benefit
perceptions would be negatively related.
...
\multicolumn{1}{>{\raggedright}m{\dimexpr 1.81in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{8}{8}\selectfont{Social\ expenditure\ (standardized)}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.71in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{8}{8}\selectfont{}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.71in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{8}{8}\selectfont{}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.71in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{8}{8}\selectfont{}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.71in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{8}{8}\selectfont{}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.71in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{8}{8}\selectfont{}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.71in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{8}{8}\selectfont{}}} & \multicolumn{1}{>{\raggedright}m{\dimexpr 0.75in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{8}{8}\selectfont{0.071}}} & \multicolumn{1}{>{\raggedright}m{\dimexpr 0.75in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{8}{8}\selectfont{0.150}}} \\
...
\multicolumn{9}{>{\raggedright}m{\dimexpr 7.56in+16\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{7}{7}\selectfont{Note:\ Cells\ contain\ regression\ coefficients\ with\ standard\ errors\ in\ parentheses.\ *\ p\ <\ 0.05;\ **\ p\ <\ 0.01;\ ***\ p\ <\ 0.001.\ Model\ 5\ to\ 8\ controls\ for\ sex,\ age,\ education\ and\ income;\ sex,\ age\ and\ income\ show\ significant\ effects,\ education\ does\ not.}}} \\
...
Table~\ref{tbl-modelos} follows a block-building logic --- type of
employment (Model 1), concern about job and income loss (Model 2), the
...
Between the first and the final model the country-level variance
declines from 0.235 to 0.193 and the BIC from 50,264.9 to 48,616.2.
...
self-employment (Model 8), platform work (Model 9), concern about job
and income loss (Model 10), technological displacement (Model 11) and
deservingness (Model 12). The BIC varies little across these models
(48,612.2--48,626.8) and the country variance is stable (≈ 0.19). The
...
conservative regimes (× conservative = −0.090, p \textless{} 0.01), with
...
(−0.120, p \textless{} 0.001). Once these interactions are specified,
the social-democratic main effect becomes significant in the
displacement and deservingness models (0.843, p \textless{} 0.01; 0.638,
p \textless{} 0.05), where it was null in Table~\ref{tbl-modelos}.
...
risk, expected to erode justice, instead increased it (H3). The
...
risk and perceived redistributive justice (H3), echoed by perceived
skill obsolescence, is the more consequential departure, and its failure
to follow a strong prior is itself informative. Conceptually, the
dependent variable is an evaluation of the justice of one's own
exchange, not a demand for redistribution. The literature behind H3
establishes that perceived risk raises the demand for protection
(\citeproc{ref-busemeyer_social_2022}{Busemeyer and Sahm, 2022};
\citeproc{ref-iversen_asset_2001}{Iversen and Soskice, 2001};
\citeproc{ref-thewissen_automation_2019}{Thewissen and Rueda, 2019}),
...
positively correlated with each other (r = 0.72) and with the outcome,
...
The comparative results recast the regime argument. Regime type did not
order perceived redistributive justice as a main effect (H6), consistent
with the heterogeneity already visible in Figure~\ref{fig-cleveland} and
at odds with the classic regime hypothesis
(\citeproc{ref-jaeger_welfare_2006}{Jæger, 2006};
\citeproc{ref-svallfors_worlds_1997}{Svallfors, 1997}). Every regime
interaction is negative, though the pattern only partly matches the
universalist--residual expectation: the positive association of platform
work is steepest under liberal arrangements and flattest under
social-democratic ones, whereas the self-employment effect, predicted to
be strongest in residual regimes, emerges only under social-democratic
arrangements (H7); the displacement effect is dampened in both
non-liberal regimes (H8); and the deservingness effect is steepest in
conservative regimes and indistinguishable between liberal and
social-democratic ones (H9). For platform work and the technological
...
raises rather than lowers it. Above all, universalist welfare
institutions appear to decouple perceived redistributive justice from
platform work and technological perception, while residual arrangements
tie them tightly together; self-employment, by contrast, becomes
consequential precisely under universalist arrangements.
...
these attitudes lies less in differences of level than in differences of
structuring: regimes shape how strongly individual circumstances
translate into judgements of justice. Reframing the regime effect as one
of decoupling rather than elevation offers a more accurate account of
why, in some institutional settings but not others, the legitimacy of
the welfare state fragments along the contours of the labour market.
...
perception from engagement with technological change as a frame. At the
...
Comparative leverage would be improved by replacing categorical regimes
with continuous indicators of decommodification and of active
labour-market and social-investment provision, which would also permit a
more direct test of whether policy feedback, rather than reassurance,
underlies the institutional patterning observed here
(\citeproc{ref-fan_how_2024}{Fan et al., 2024};
\citeproc{ref-gallego_automation_2022}{Gallego and Kurer, 2022}).
